#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 Wayback 历史官网采集崩坏：星穹铁道 Spine 三件套资源。"""
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数。"""
    parser = argparse.ArgumentParser(description="Collect Star Rail spine assets from Wayback snapshots.")
...
def parse_atlas_png_url(atlas_text: str, atlas_url: str) -> str:
    ...
    for line in atlas_text.splitlines():
        ...
        if value.lower().endswith(".png"):
            return urljoin(atlas_url, quote(value, safe="/:%#?=&"))
...
def save_asset_group(output_dir: Path, group: dict[str, object]) -> None:
    ...
    (output_dir / str(group["png_name"])).parent.mkdir(parents=True, exist_ok=True)
    (output_dir / str(group["png_name"])).write_bytes(group["png"])
...
def collect_asset_group(
    ...
    try:
        ...
        png_data = fetch_bytes(png_url, timeout=timeout)
    ...
    if not is_spine_json(json_data):
        ...
        return None
    if not is_png(png_data):
        log(f"跳过非 PNG 图片: {png_url}")
        return None
